...
rgb Pointlight::getDiffuse(const Vector3& camera, HitRecord& hit)const{
	Vector3 dir = location - hit.location;
	double dist = dir.length();
	dir = unitVector(dir);
	rgb color = light/(pow(dist,2));
...
rgb Pointlight::getSpecular(const Vector3& camera, HitRecord& hit)const{
	Vector3 dir = location - hit.location;
...
double Pointlight::getShadow(const Vector3& ray, HitRecord& hit, vector<Shape*> shapes)const{
	//determine 
